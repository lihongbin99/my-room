...
src/
  ...
  Experience/
    ...
    Navigation.js                # 相机拖拽/缩放/平移，移植自 my-room-in-3d 的 Navigation.js
...
- **性能**：pixelRatio 上限 1.5（`Sizes.js`）、阴影贴图 1024、renderer `powerPreference: 'high-performance'`。用户反馈过卡顿，加重型效果（Bloom、更多阴影灯）前先想想帧率。
...
实施顺序建议：先做 CSS3D + 聚焦相机的基础设施（功能 4 的壳），XP、马里奥、ChatGPT 复用同一套机制。
