...
src/
  ...
  Experience/
    ...
    World/
      ...
      TVZone.js                  # 电视区（电视柜/电视/Switch/沙发/茶几/一对游戏手柄，6 个独立 GLB；手柄是 gamepads.glb 一白一黑 Xbox 风格并排平放茶几右前，经 crush-glb 压缩带 meshopt）
...
- **BIOS 加载屏**（TODO 8，2026-07 完成）：`Loading.js` + index.html 里的静态遮罩标记（JS 到位前就显示第一行；z-index 10010 盖住 stats.js 的 10000）。黑底绿字仿 POST 自检逐行打印真实加载日志，加载完自动淡出进场（用户明确不要 START 门）。三个机制：① **日志行**挂 `THREE.DefaultLoadingManager.onProgress`（各模块 loader 都没传自定义 manager，天然全走它；同一 GLB 触发两次——下载完+解析完——seen 集去重；GLB 内嵌贴图以 blob: URL 过 manager，合并一行批量计数）。**懒加载资源必须隔离**：Bookshelf 封面已改私有 `new THREE.LoadingManager()`，否则进场后取书会让日志"复活"，新懒加载资源照办。② **完成收口不用 manager.onLoad**（GLTF 解析间隙会误触发，且 XPScreen 截图纹理在 GLB 解析回调里才开始加载）——各模块暴露 `ready` Promise（ComputerZone 含 XPScreen、TVZone、MarioTV ROM），`Loading.start()` 依次 await；**约定：加载失败也要 resolve**（打 FAIL/NOT FOUND 行），绝不能卡死开机；新增加载类模块记得暴露 ready 并加进 start()。③ **shader 预热**：遮罩盖着时用 `Environment.pin(mix)` 把 currentMix 逐状态钉住各渲染一帧（1→0.08 粗步进 ×0.7，0.08→0 细步进 0.004），把所有"灯数×管线"组合的全场景 shader 变体编译掉——灯的 visible 一变（FloorLamp spot >0.00625、DeskGlow 台灯 >0.0385、composer >0.02）就是一整套新变体，实测每套 20+ 个程序、编译冻结 ≈4s（RTX 5060）。**⚠️ 教训（2026-07 重修过）**：旧方案靠自然缓动往返、指望回程"在阈值带内必然多帧停留"，但缓动是 `min(1, 0.005×delta)` 的指数追赶——编译冻结帧 delta 数秒，下一步直接跳过低 mix 阈值带，漏掉的变体全堆到首次真实拖滑杆时爆 3.8s 冻结（材质越加越多后必现，"编译卡顿自己把下一段编译踹漏"）；钉步进不受冻结影响，卡多久都只是慢不会跳步。以后新增夜灯开关阈值须 <0.08（`intensity > 0.05` 约定下夜间强度 ≥0.7 即满足），否则在粗段补一步。进场后首拖仅剩 3 个异步纹理残留变体（50ms 级，Playwright 实测 110→113）。勿换成 `compileAsync` 单点预热：盖不住 shadow pass 和 composer 变体。本地全缓存开机全程 ≈18s（隐藏编译约 8s + 钉步进 ≈1s + 日志节奏），慢服务器上下载期间日志本来就在滚，表演时间与下载重叠不白等。
...
8. [x] **BIOS 加载屏**（2026-07 完成）：`Loading.js` 仿开机自检——真实资源日志逐行打印、各模块 ready Promise 收口、逐状态钉住 currentMix 的 shader 预热（首次拖滑杆的编译卡顿消化进 BIOS）、加载完自动淡出进场（无 START 门，用户已定）。机制与陷阱见"关键实现说明"的"BIOS 加载屏"条。遗留：
